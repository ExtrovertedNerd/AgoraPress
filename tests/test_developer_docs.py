...
def test_catalog_covers_forum_moderation(docs_root: Path) -> None:
    """SPEC: catalog rows for last-post recount, move, merge, split, report."""
    text = (docs_root / "features_and_functions.md").read_text(encoding="utf-8")
    assert re.search(r"(?im)^##\s+Forum moderation\s*$", text), (
        "features_and_functions.md missing heading: Forum moderation"
    )
    for needle in (
        ...
        "listMergeTargets",
        "userCanSplitTopic",
        "listSplitDestinations",
        "ap_forum_user_can_move_topic",
        "ap_forum_move_destinations",
        "ap_forum_move_topic_form_html",
        "ap_forum_user_can_merge_topic",
        "ap_forum_merge_targets",
        "ap_forum_merge_topic_form_html",
        "ap_forum_user_can_split_topic",
        "ap_forum_split_destinations",
