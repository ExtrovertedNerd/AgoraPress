 ...
 * Data via {@see zeroshits_get_forum_index_data()} / filter `zeroshits_forum_index_data`.
 *
 * @package ZeroShits
 */
...
$categories = function_exists('zeroshits_get_forum_index_data') ? zeroshits_get_forum_index_data() : [];
...
<div class="ap-forum ap-forum--index">
    <header class="ap-forum__header">
        <div>
            <h1 class="ap-archive-title">The Stalls (Forums)</h1>
            <p class="ap-forum__lead">Pick a door, pick a fight, pick a dumb take. We give zero shits either way.</p>
        </div>
    </header>

<?php
$searchAction = function_exists('zeroshits_forum_search_url')
    ? zeroshits_forum_search_url()
    ...
    <?php else : ?>
        <?php foreach ($categories as $category) : ?>
            ...
            <section class="ap-forum-panel">
                <h2 class="ap-forum-panel__title"><?php echo zeroshits_esc($catName); ?></h2>
            <?php if ($forums === []) : ?>
                <?php
                echo function_exists('ap_forum_empty_state_html')
                        ...
                        <li class="<?php echo zeroshits_esc_attr($rowClass); ?>">
                            ...
                            <div class="ap-forum-list__main ap-forum-row__title">
                                <h3 class="ap-forum-list__name">
                                    ...
                                    <?php endif; ?>
                                    <a href="<?php echo zeroshits_esc_url($url); ?>"><?php echo zeroshits_esc($name); ?></a>
                                </h3>
                                <?php if ($desc !== '') : ?>
                                    <p class="ap-forum-list__desc"><?php echo zeroshits_esc($desc); ?></p>
                                <?php endif; ?>
                            </div>
                            <?php // SPEC A4 col 3: Topics ?>
                            <div class="ap-forum-stat ap-forum-list__topics ap-forum-row__topics" aria-label="<?php echo zeroshits_esc_attr($topics . ' topics'); ?>">
