...
$forumsUrl = function_exists('ap_forums_url') ? ap_forums_url() : (function_exists('zeroshits_home_url') ? zeroshits_home_url('/forums/') : '/forums/');
$searchAction = function_exists('zeroshits_forum_search_url') ? zeroshits_forum_search_url() : $forumsUrl;
$data = function_exists('zeroshits_get_forum_search_data') ? zeroshits_get_forum_search_data() : ['query' => '', 'total' => 0, 'results' => []];
$term = (string) ($data['query'] ?? '');
$total = (int) ($data['total'] ?? 0);
$results = is_array($data['results'] ?? null) ? $data['results'] : [];
$notice = function_exists('ap_get_forum_notice') ? ap_get_forum_notice() : null;
if ($notice === null && function_exists('zeroshits_get_forum_notice')) {
    $notice = zeroshits_get_forum_notice();
}
...
<?php else : ?>
    <form class="ap-search-form ap-forum-search-form" role="search" method="get" action="<?php echo zeroshits_esc_url($searchAction); ?>">
        ...
        <?php endif; ?>
        <input type="search" id="ap-forum-search-field" name="forum_s" value="<?php echo zeroshits_esc_attr($term); ?>" placeholder="Search topics and posts…" required>
    ...
    <?php elseif ($results === []) : ?>
        <div class="ap-empty" role="status">
            <p>No results matched “<?php echo zeroshits_esc($term); ?>”.</p>
        </div>
    <?php else : ?>
        <ul class="ap-forum-search-results">
            <?php foreach ($results as $row) : ?>
                ...
                ?>
                <li class="ap-forum-search-result ap-forum-search-result--<?php echo zeroshits_esc_attr($rtype); ?>">
                    <span class="ap-forum-search-result__type"><?php echo zeroshits_esc($label); ?></span>
                    <a class="ap-forum-search-result__title" href="<?php echo zeroshits_esc_url($url); ?>"><?php echo zeroshits_esc($title); ?></a>
                    <?php if ($snippet !== '') : ?>
                        <p class="ap-forum-search-result__snippet"><?php echo zeroshits_esc($snippet); ?></p>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
