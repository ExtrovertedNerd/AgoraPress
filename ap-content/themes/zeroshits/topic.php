 ...
 * Data via {@see zeroshits_get_topic_posts_data()} / filter `zeroshits_topic_posts_data`.
 *
 * @package ZeroShits
 */
...
$posts = function_exists('zeroshits_get_topic_posts_data')
    ? zeroshits_get_topic_posts_data($topicId)
    : [];
...
<nav class="ap-breadcrumbs" aria-label="Breadcrumb">
    <ol>
        ...
        <li><a href="<?php echo zeroshits_esc_url($forumUrl); ?>"><?php echo zeroshits_esc($forumName); ?></a></li>
        <li><span aria-current="page"><?php echo zeroshits_esc($topicTitle); ?></span></li>
    </ol>
</nav>
...
<article class="ap-forum ap-forum--topic" id="ap-topic-top">
    <header class="ap-forum__header">
        <div>
            <h1 class="ap-forum-topic-title"><?php echo zeroshits_esc($topicTitle); ?></h1>
            <p class="ap-forum-topic-meta">
                ...
                <?php endif; ?>
                <span><?php echo zeroshits_esc($forumName); ?></span>
                <?php if ($posts !== []) : ?>
...
<?php elseif ($notFound) : ?>
    <div class="ap-empty ap-forum-empty ap-forum-empty--topic_not_found" role="status">
        <p>Topic not found.</p>
        <p class="ap-forum-empty__back"><a href="<?php echo zeroshits_esc_url($forumsUrl); ?>">Back to forums</a></p>
    </div>
<?php elseif ($posts === []) : ?>
    <?php
    echo function_exists('ap_forum_empty_state_html')
    ...
    <div class="ap-forum-posts">
        <?php foreach ($posts as $index => $post) : ?>
            ...
            if (!empty($post['signature_html']) && is_string($post['signature_html'])) {
                ...
                if (function_exists('zeroshits_esc')) {
                    $signatureHtml = nl2br(zeroshits_esc($sigRaw), false);
                } else {
            ...
            ?>
            <section class="ap-forum-post ap-forum-post--two-pane" id="post-<?php echo $postId; ?>" aria-label="<?php echo zeroshits_esc_attr('Post #' . $postNum . ' by ' . $author); ?>">
                <?php // Left pane — author (SPEC B2): avatar, name, role, posts, likes, joined, location. ?>
                <aside class="ap-forum-post__author" aria-label="<?php echo zeroshits_esc_attr('Author: ' . $author); ?>">
                    <div class="ap-forum-post__avatar" aria-hidden="true">
                        <?php if ($avatarHtml !== '') : ?>
                            <?php echo $avatarHtml; ?>
                        <?php else : ?>
                            <span class="ap-forum-post__avatar-fallback"><?php echo zeroshits_esc($initial); ?></span>
                        <?php endif; ?>
                    </div>
                    <p class="ap-forum-post__author-name">
                        <?php if ($authorUrl !== '') : ?>
                            <a href="<?php echo zeroshits_esc_url($authorUrl); ?>" class="ap-forum-post__author-link"><?php echo zeroshits_esc($author); ?></a>
                        <?php else : ?>
                            <?php echo zeroshits_esc($author); ?>
                        <?php endif; ?>
                    </p>
                    <?php if ($role !== '') : ?>
                        <p class="ap-forum-post__author-role ap-forum-post__author-meta"><?php echo zeroshits_esc($role); ?></p>
                    <?php endif; ?>
                        ...
                        <dl class="ap-forum-post__author-stats ap-forum-post__stats">
                            ...
                            <?php if ($joinedDisplay !== '') : ?>
                                <div class="ap-forum-post__stat ap-forum-post__stat--joined">
                                    <dt>Joined</dt>
                                    <dd>
                                        <time class="ap-forum-post__joined" datetime="<?php echo zeroshits_esc_attr($joinedRaw); ?>"><?php echo zeroshits_esc($joinedDisplay); ?></time>
                                    </dd>
                                </div>
                            <?php endif; ?>
                            <?php if ($location !== '') : ?>
                                <div class="ap-forum-post__stat ap-forum-post__stat--location">
                                    <dt>Location</dt>
                                    <dd class="ap-forum-post__location"><?php echo zeroshits_esc($location); ?></dd>
                                </div>
                            <?php endif; ?>
                        </dl>
                    <?php endif; ?>
                ...
                <div class="ap-forum-post__main">
                    <div class="ap-forum-post__head">
                        <div class="ap-forum-post__head-start">
                            <?php if ($subject !== '') : ?>
                                <span class="ap-forum-post__subject"><?php echo zeroshits_esc($subject); ?></span>
                            <?php elseif ($date !== '') : ?>
                                <time class="ap-forum-post__date" datetime="<?php echo zeroshits_esc_attr($date); ?>"><?php echo zeroshits_esc($date); ?></time>
                            <?php else : ?>
                        ...
                        <?php if ($hasActions && !$isEditing) : ?>
                            <div class="ap-forum-post__actions" role="group" aria-label="Post actions">
                                <?php if ($canQuote) : ?>
                                    <a class="ap-btn ap-btn--ghost ap-btn--sm ap-forum-quote" href="?quote=<?php echo (int) $postId; ?>#reply" aria-label="<?php echo zeroshits_esc_attr('Quote post #' . $postNum); ?>">Quote</a>
                                <?php endif; ?>
                                <?php if ($canEdit) : ?>
                                    <a class="ap-btn ap-btn--ghost ap-btn--sm ap-forum-edit" href="?edit_post=<?php echo (int) $postId; ?>#post-<?php echo (int) $postId; ?>" aria-label="<?php echo zeroshits_esc_attr('Edit post #' . $postNum); ?>">Edit</a>
                                <?php endif; ?>
                                <?php if ($canDelete) : ?>
                                    <form method="post" action="" class="ap-forum-action-form ap-forum-action-form--inline" onsubmit="return confirm('Delete this post?');">
                                        ...
                                        ?>
                                        <button type="submit" class="ap-btn ap-btn--ghost ap-btn--sm ap-btn--danger ap-forum-delete" aria-label="<?php echo zeroshits_esc_attr('Delete post #' . $postNum); ?>">Delete</button>
                                    </form>
                                <?php endif; ?>
                                <?php if ($canLike || $likeCount > 0) : ?>
                                    <?php if ($canLike) : ?>
                                        <form method="post" action="" class="ap-forum-action-form ap-forum-action-form--inline">
                                            ...
                                            ?>
                                            <button type="submit" class="ap-btn ap-btn--ghost ap-btn--sm ap-forum-like<?php echo $likedByMe ? ' is-liked' : ''; ?>" aria-pressed="<?php echo $likedByMe ? 'true' : 'false'; ?>" aria-label="<?php echo zeroshits_esc_attr($likeLabel); ?>">
                                    ...
                                    <?php elseif ($likeCount > 0) : ?>
                                        <span class="ap-forum-like ap-forum-like--static" aria-label="<?php echo zeroshits_esc_attr($likeCount . ' like' . ($likeCount === 1 ? '' : 's')); ?>">
                    ...
                    <?php if ($isEditing) : ?>
                        <form method="post" action="" class="ap-forum-form ap-forum-form--edit">
                            ...
                            <div class="ap-field">
                                ...
                                if (function_exists('ap_editor')) {
                                    ...
                                    echo '<textarea id="edit-body-' . (int) $postId . '" name="post_body" required rows="6">'
                                        . zeroshits_esc($rawContent) . '</textarea>';
                                }
                                ?>
                            </div>
                            <div class="ap-field">
                            ...
                            <div class="ap-forum-toolbar">
                                ...
                                echo zeroshits_esc_url(strtok((string) ($_SERVER['REQUEST_URI'] ?? ''), '?') . '#post-' . $postId);
                                ?>">Cancel</a>
                            </div>
                        </form>
                    <?php else : ?>
                        ...
                        <ul class="ap-forum-attachments" aria-label="Attachments">
                            <?php foreach ($attachments as $att) : ?>
                                ...
                                <li class="ap-forum-attachments__item">
                                    <a href="<?php echo zeroshits_esc_url($attUrl); ?>"><?php echo zeroshits_esc($attName); ?></a>
                                    <?php if ($attSize !== '') : ?>
                                        <span class="ap-forum-attachments__size"><?php echo zeroshits_esc((string) $attSize); ?></span>
                                    <?php endif; ?>
                                </li>
                            <?php endforeach; ?>
                        </ul>
        ...
        <section class="ap-forum-form" id="reply" aria-labelledby="reply-heading">
            ...
            <form method="post" action="">
                ...
                <div class="ap-field">
                    ...
                    <textarea id="agora-reply-body" name="reply_body" required rows="6" placeholder="Write your reply…"><?php echo zeroshits_esc($quoteMarkup); ?></textarea>
                        <?php
                    }
                    ?>
                </div>
