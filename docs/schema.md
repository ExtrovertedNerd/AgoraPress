...
This is the **schema guide** for AgoraPress **`0.3.8-beta`** (schema `AP_DB_VERSION` **12**). It describes tables, numbered migrations, the configurable prefix (default **`ap_`**), and multi-driver notes **as built**.
...
**Target schema version:** `AP_DB_VERSION` in `ap-includes/version.php` (currently **12**, AgoraPress `0.3.8-beta`)
