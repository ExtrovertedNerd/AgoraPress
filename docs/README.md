...
**Current core:** `AP_VERSION` **0.3.6-beta** · schema `AP_DB_VERSION` **12** · product status: MVP feature-complete with Site Icon (favicon pack), plugin admin pages (ACP registry + router), forum UI phpBB-parity, forum likes/moderation, Theme Options API, and local opt-in analytics; ready for live-site install.
