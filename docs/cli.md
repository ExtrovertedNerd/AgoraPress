...
This is the **installed-site CLI cookbook** for AgoraPress **`0.3.11-beta`**
(schema `AP_DB_VERSION` **13**). It describes every built-in command group
...
| `--skip-themes` | Defines `AP_CLI_SKIP_THEMES`. Help text: “Skip theme setup side effects where possible.” As of 0.3.11-beta, core bootstrap does **not** read this constant (only `AP_CLI_SKIP_PLUGINS` is branched on); the flag is accepted and reserved. |
...
Stdout: `AgoraPress 0.3.11-beta (PHP 8.x.x)`. Loads `ap-includes/version.php`
...
```text
db_version_target: 13
db_version_applied: 13
```
...
remote: 0.3.11-beta
update: none
download: https://agorapress.extrovertednerd.com/download/AgoraPress-0.3.11-beta.zip
...
`AP_DB_VERSION` **13**. Already current:

```text
No pending migrations (schema at 13).
```
